...
get_min_imap = function(corr_mat, n, perm, alpha){
...
update_IMAP = function(g_hat, new_perm, prev_perm, j, corr_mat, n, alpha_level){
  p = nnodes(g_hat)
  e = empty.graph(as.character(1:p))
  inc_mat = get_incident_mat(g_hat)
  pi_j = new_perm[j]
  pi_jp1 = new_perm[j+1]
  S_j = new_perm[1:(j-1)]
  S_jp1 = new_perm[1:j]
...
log_prob = function(g, data, interventions){
...
log_prob_init = function(data, interventions, corr_mat, n, pi, alpha, gamma){
  pi_imap_hat = get_min_imap(corr_mat, n, pi, alpha)
...
log_prob_cached = function(g_prev, pi_new, pi_prev, j, data, interventions, corr_mat, n, alpha, gamma){
  pi_imap_hat = update_IMAP(g_prev,pi_new,pi_prev,j, corr_mat, n, alpha)
  data_dep_prior = -gamma * narcs(pi_imap_hat)
  p_lik = log_prob(pi_imap_hat, data, interventions)
  return(list(p_lik + data_dep_prior, pi_imap_hat))
}
...
minIMAP_MCMC = function(data_path, intervention_path, alpha=.05, gamma=1, n_iter=500, save_step=100, path='../data/TEMP_DAGS/'){
  ...
  pi_prev = pi_0
  accepted = 0
  prev = log_prob_init(data, interventions, corr_mat, n, pi_prev, alpha, gamma) # log scale
  p_prev = prev[[1]]
  emp_dag_prev = prev[[2]]
  scores = c()
  emp_dags = list()
  for (i in 1:n_iter){
    ...
    if(x_prop[p] != pi_prev[1]){ # fast update only works if transposition doesn't occur at end
      curr = log_prob_cached(emp_dag_prev, x_prop, pi_prev, j, data, interventions, corr_mat, n, alpha, gamma) # log scale
    } else{
      curr = log_prob_init(data, interventions, corr_mat, n, x_prop, alpha, gamma)
    }
    p_prop = curr[[1]]
    emp_dag_curr = curr[[2]]
